...
class _MatchDetailScreenState extends State<MatchDetailScreen> {
        ...
        children: [
          ...
          Glass(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  'TODO(backend): Add file picker + upload. For MVP this just changes match to "Under Review".',
          ...
          Glass(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Organizer review dashboard (mock)',
                    style: Theme.of(context)
                        .textTheme
                        .titleMedium
                        ?.copyWith(fontWeight: FontWeight.w800)),
  ...
  Future<void> _review(bool approve) async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => _status = approve ? 'Completed' : 'Pending Proof');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(approve ? 'Approved (mock).' : 'Rejected (mock).')),
      );
    } finally {
      if (mounted) setState(() => _busy = false);
    }
